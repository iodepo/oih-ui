...
import iconOcean from "../resources/logos/logoOcean.svg";

const Export = (props) => {
  const { uri, searchType, resultCount } = props;
    ...
    if (URI !== "" && resultCount !== 0) {
      ...
      fetch(URI)
        ...
        .then((json) => {
          ...
          const newJson = createObjectSchema(docs);
          download(format, newJson, searchType);
        });
    }
  };

  const createObjectSchema = (docs) => {
    switch (searchType) {
      case "Person":
        return docs.map((d) => {
          return {
            name: d.name,
            jobTitle: d.txt_jobTitle,
            knowsAbout: d.txt_knowsAbout,
            knowsLanguage: d.txt_knowsLanguage,
            nationality: d.txt_nationality,
          };
        });
      case "CreativeWork":
        return docs.map((d) => {
          return {
            ...
            id: d.id,
            author: d.txt_author,
            keywords: d.txt_keywords,
            contributors: d.txt_contributor,
      ...
      case "Course":
        return docs.map((d) => {
          return {
            name: d.name,
            courseInstance: d.txt_hasCourseInstance,
            location: d.txt_location,
            description: d.description,
          };
        });
      case "SpatialData":
      case "Dataset":
        return docs.map((d) => {
          return {
            name: d.name,
            sameAs: d.txt_sameAs,
            license: d.txt_license,
            relatedWorks: d.txt_citation,
            version: d.txt_version,
            keywords: d.txt_keywords,
            dataCatalog: d.id_includedInDataCatalog,
            temporalCoverage: d.txt_temporalCoverage,
            distribution: d.txt_distribution,
            region: d.txt_region,
            providerID: d.id_provider,
            providers: d.txt_provider,
            description: d.description,
          };
        });
      case "Vehicle":
        return docs.map((d) => {
          return {
            name: d.name,
            additionalProperty: d.txt_additionalProperty,
            category: d.txt_category,
            configuration: d.txt_vehicleConfiguration,
            specialUsage: d.txt_vehicleSpecialUsage,
            description: d.description,
          };
        });
      case "ResearchProject":
        return docs.map((d) => {
          return {
            name: d.name,
            alternateName: d.txt_identifier,
            identifier: d.txt_identifier,
            memberOf: d.txt_memberOf,
            parentOrg: d.txt_parentOrganization,
            knowsAbout: d.txt_knowsAbout,
            providerID: d.id_provider,
            providers: d.txt_provider,
            description: d.description,
  ...
  const download = async (format, data, searchType) => {
    ...
    switch (format) {
      ...
      case "csv":
        ...
        break;
      case "pdf":
        const doc = createPDF(data, searchType);
  ...
  const createPDF = (data, searchType) => {
    let doc;

    switch (searchType) {
      case "Person":
        doc = createPersonPDF(data);
        break;
      case "CreativeWork":
        doc = createCreativeWorkPDF(data);
        break;
      case "Course":
        doc = createCoursePDF(data);
        break;
      case "SpatialData":
      case "Dataset":
        doc = createDatasetPDF(data);
        break;
      case "Vehicle":
        doc = createVehiclePDF(data);
        break;
      case "ResearchProject":
        doc = createResearchProjectPDF(data);
        break;
      case "Organization":
        doc = createOrganizationPDF(data);
        break;
    }
  ...
  const createPersonPDF = (data) => {
    return (
      <Document title="Experts Data">
        <Page size="A4" style={styles.page}>
          <View fixed>
            <SvgOceanLogo />
            <Text>{"\n"}</Text>
          </View>
          <Text style={styles.title}>Experts Data:</Text>

          <View style={styles.section}>
            {data.map((d) => (
              <>
                {d.name && <Text style={styles.boldTitle}>{d.name}</Text>}
                {d.jobTitle && (
                  <Text>
                    <Text style={styles.boldCategory}>Job Title</Text>:{" "}
                    {d.jobTitle}
                  </Text>
                )}
                {d.knowsAbout && (
                  <Text>
                    <Text style={styles.boldCategory}>Knows About</Text>:{" "}
                    {d.knowsAbout.join(", ")}
                  </Text>
                )}
                {d.knowsLanguage && (
                  <Text>
                    <Text style={styles.boldCategory}>Knows Language</Text>:{" "}
                    {d.knowsLanguage.join(", ")}
                  </Text>
                )}
                {d.nationality && (
                  <Text>
                    <Text style={styles.boldCategory}>Nationality</Text>:{" "}
                    {d.nationality}
                  </Text>
                )}
                <Text>{"\n"}</Text>
  ...
  const createCreativeWorkPDF = (data) => {
    return (
      <Document title="Documents Data">
        <Page size="A4" style={styles.page}>
          <View fixed>
            <SvgOceanLogo />
            <Text>{"\n"}</Text>
          </View>
          <Text style={styles.title}>Documents Data:</Text>

          <View style={styles.section}>
            {data.map((d) => (
              <>
                {d.title && <Text style={styles.boldTitle}>{d.title}</Text>}
                {d.id && (
                  <Text>
                    <Text style={styles.boldCategory}>ID:</Text> {d.id}
                  </Text>
                )}
                {d.author && (
                  <Text>
                    <Text style={styles.boldCategory}>Author</Text>: {d.author}
                  </Text>
                )}
                {d.keywords && (
                  <Text>
                    <Text style={styles.boldCategory}>Keywords</Text>:{" "}
                    {d.keywords.join(", ")}
                  </Text>
                )}
                {d.contributors && (
                  <Text>
                    <Text style={styles.boldCategory}>Contributors</Text>:{" "}
                    {d.contributors.join(", ")}
                  </Text>
                )}
                {d.description && (
                  <Text>
                    <Text style={styles.boldCategory}>Description</Text>:{" "}
                    {d.description}
                  </Text>
                )}
                <Text>{"\n"}</Text>
          ...
          <View style={styles.section}>
            {data.map((d) => (
              <>
                ...
                {d.courseInstance && (
                  <Text>
                    <Text style={styles.boldCategory}>Course Instance:</Text>{" "}
                    {d.courseInstance}
                  </Text>
                )}
                {d.location && (
                  <Text>
                    <Text style={styles.boldCategory}>Location</Text>:{" "}
                    {d.location}
                  </Text>
                )}
                {d.description && (
  ...
  function createDatasetPDF(data) {
    return (
      <Document title="Datasets Data">
        <Page size="A4" style={styles.page}>
          <View fixed>
            <SvgOceanLogo />
            <Text>{"\n"}</Text>
          </View>
          <Text style={styles.title}>Datasets Data:</Text>
          ...
          <View style={styles.section}>
            {data.map((d) => (
              <>
                ...
                {d.sameAs && (
                  <Text>
                    <Text style={styles.boldCategory}>Same as</Text>: {d.sameAs}
                  </Text>
                )}
                {d.license && (
                  <Text>
                    <Text style={styles.boldCategory}>License</Text>:{" "}
                    {d.license}
                  </Text>
                )}
                {d.keywords && (
                ...
                {d.dataCatalog && (
                  <Text>
                    <Text style={styles.boldCategory}>Data Catalog</Text>:{" "}
                    {d.dataCatalog}
                  </Text>
                )}
                {d.temporalCoverage && (
                  <Text>
                    <Text style={styles.boldCategory}>Temporal Coverage</Text>:{" "}
                    {d.temporalCoverage}
                  </Text>
                )}
                {d.region && (
                  <Text>
                    <Text style={styles.boldCategory}>Region</Text>: {d.region}
                  </Text>
                )}
                {d.distribution && (
                  <Text>
                    <Text style={styles.boldCategory}>Distribution</Text>:{" "}
                    {d.distribution.join(", ")}
                  </Text>
                )}
                {d.relatedWorks && (
                  <Text>
                    <Text style={styles.boldCategory}>Related Works</Text>:{" "}
                    {d.relatedWorks.join(", ")}
                  </Text>
                )}
                {d.providerID && (
                  <Text>
                    <Text style={styles.boldCategory}>Provider ID</Text>:{" "}
                    {d.providerID.join(", ")}
                  </Text>
                )}
                {d.providers && (
                  <Text>
                    <Text style={styles.boldCategory}>Providers</Text>:{" "}
                    {d.providers.join(", ")}
                  </Text>
                )}
                {d.description && (
  ...
  function createVehiclePDF(data) {
    return (
      <Document title="Vessels Data">
        <Page size="A4" style={styles.page}>
          <View fixed>
            <SvgOceanLogo />
            <Text>{"\n"}</Text>
          </View>
          <Text style={styles.title}>Vessels Data:</Text>
          ...
          <View style={styles.section}>
            {data.map((d) => (
              <>
                ...
                {d.category && (
                  <Text>
                    <Text style={styles.boldCategory}>Category</Text>:{" "}
                    {d.category}
                  </Text>
                )}
                {d.configuration && (
                  <Text>
                    <Text style={styles.boldCategory}>Configuration</Text>:{" "}
                    {d.configuration}
                  </Text>
                )}
                {d.specialUsage && (
                  <Text>
                    <Text style={styles.boldCategory}>Special Usage</Text>:{" "}
                    {d.specialUsage}
                  </Text>
                )}
                {d.description && (
  ...
  function createResearchProjectPDF(data) {
    return (
      <Document title="Research Project Data">
        <Page size="A4" style={styles.page}>
          <View fixed>
            <SvgOceanLogo />
            <Text>{"\n"}</Text>
          </View>
          <Text style={styles.title}>Research Project Data:</Text>
          ...
          <View style={styles.section}>
            {data.map((d) => (
              <>
                ...
                {d.alternateName && (
                  <Text>
                    <Text style={styles.boldCategory}>Alternate Name:</Text>{" "}
                    {d.alternateName}
                  </Text>
                )}
                {d.identifier && (
                  <Text>
                    <Text style={styles.boldCategory}>Identifier</Text>:{" "}
                    {d.identifier}
                  </Text>
                )}
                {d.memberOf && (
                  <Text>
                    <Text style={styles.boldCategory}>Member of</Text>:{" "}
                    {d.memberOf}
                  </Text>
                )}
                {d.parentOrg && (
                  <Text>
                    <Text style={styles.boldCategory}>Parent Organization</Text>
                    : {d.parentOrg}
                  </Text>
                )}
                {d.knowsAbout && (
  ...
  function createOrganizationPDF(data) {
    return (
      <Document title="Institutions Data">
        <Page size="A4" style={styles.page}>
          <View fixed>
            <SvgOceanLogo />
            <Text>{"\n"}</Text>
          </View>
          <Text style={styles.title}>Institutions Data:</Text>
          ...
          <View style={styles.section}>
            {data.map((d) => (
              <>
                ...
                {d.url && (
                  <Text>
                    <Text style={styles.boldCategory}>Url:</Text> {d.url}
                  </Text>
                )}
                {d.telephone && (
                  <Text>
                    <Text style={styles.boldCategory}>Telephone</Text>:{" "}
                    {d.telephone}
                  </Text>
                )}
                {d.memberOf && (
  ...
  return (
    <>
      <button onClick={() => onClickExport("10", "pdf")}>Export</button>
    </>
  );
};
